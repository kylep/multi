...
| [obra/superpowers](https://github.com/obra/superpowers) | Curated skills library with a loader that pushes Claude to invoke them aggressively. See my [walkthrough of the plugin](/exploring-claude-plugin-obra-superpowers.html) for what's in it and how to use it | Always-on token cost from the SessionStart hook, which injects the [`using-superpowers` loader](https://github.com/obra/superpowers/blob/main/skills/using-superpowers/SKILL.md) (~1.3–1.5k tokens) on every session, even on tasks where no skill applies |
