...
# well under 1 MB — i.e. exactly the shape HOI4 launcher / Workshop expect.
#
# Usage:
#   1. Drop a source image at mod/thumbnail-src.<ext> (any common format)
#   2. ./scripts/process-thumbnail.sh
#   3. file mod/thumbnail.png   # confirm "PNG image data, 512 x 512, 8-bit/color RGB"
#
...
# 1. Resize to fit 512x512 (preserves aspect ratio; Workshop wants exactly
...
# Strip alpha (Workshop preview composites poorly with transparency and the
# launcher renders it badly).
...
if [ -n "${BLOG_IMG_DIR}" ]; then
  cp "${DEST}" "${BLOG_IMG_DIR}/hoi4-australia-thumbnail.png"
  echo "Wrote:"
  echo "  ${DEST}"
  echo "  ${BLOG_IMG_DIR}/hoi4-australia-thumbnail.png"
else
  echo "Wrote: ${DEST}"
