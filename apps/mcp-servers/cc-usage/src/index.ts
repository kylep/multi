...
async function getPricing(): Promise<Record<string, ModelPricing>> {
  ...
  for (const [model, data] of Object.entries(raw)) {
    ...
    pricing[model] = data as ModelPricing;
  }
...
function findModelPricing(
  ...
  if (pricing[prefixed]) return pricing[prefixed];
...
function calculateCost(usage: TokenUsage, mp: ModelPricing): number {
  let cost =
    tieredCost(usage.input_tokens, mp.input_cost_per_token, mp.input_cost_per_token_above_200k_tokens) +
    tieredCost(usage.output_tokens, mp.output_cost_per_token, mp.output_cost_per_token_above_200k_tokens) +
    tieredCost(
      usage.cache_creation_input_tokens ?? 0,
      mp.cache_creation_input_token_cost,
      mp.cache_creation_input_token_cost_above_200k_tokens
    ) +
    tieredCost(
      usage.cache_read_input_tokens ?? 0,
      mp.cache_read_input_token_cost,
      mp.cache_read_input_token_cost_above_200k_tokens
    );
...
function toMonthStr(iso: string): string {
  if (!iso) return "unknown";
  return iso.slice(0, 7); // YYYY-MM
}

async function loadAllEntries(): Promise<UsageEntry[]> {
...
function formatSummaries(summaries: DaySummary[], title: string): string {
  if (summaries.length === 0) return "No usage data found.";
  ...
  for (const s of summaries) {
    ...
    lines.push(
      `${s.date}  ${formatDollars(s.totalCost)}  ` +
...
server.tool(
  "get_usage",
  "Get Claude Code usage and estimated cost from local session logs. " +
...
server.tool(
  "get_total_spend",
  "Get total Claude Code spend across all time or a specific number of days.",
  {
    days: z
      .number()
      .optional()
