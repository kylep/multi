import { BattleRobot, TurnAction } from "./types.js";

export function planAiTurn(robot: BattleRobot): TurnAction {
  const consumablesUsed: number[] = [];
...
export function suggestPlayerAction(robot: BattleRobot): TurnAction {
  // Same logic as AI for suggestions
  return planAiTurn(robot);
}

function selectBestWeapons(robot: BattleRobot): number[] {
