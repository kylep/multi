...
export function calculateDamage(baseDamage, attack, defence) {
    const raw = baseDamage * (1 + attack / 100) - defence;
    return Math.max(0, Math.floor(raw));
}
export function resolveAttack(attacker, defender, weaponSlots, random = Math.random) {
    let atk = { ...attacker };
    let def = { ...defender };
    const results = [];
...
export function resolveRest(robot) {
...
export function resolveConsumable(user, target, consumableSlot) {
    const consumable = user.consumables[consumableSlot];
    if (!consumable)
        throw new Error("Invalid consumable slot");
...
export function validateWeaponSelection(robot, slots) {
