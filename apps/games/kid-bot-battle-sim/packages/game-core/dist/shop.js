...
export function buyItem(robot, itemName) {
    const itemDef = lookupItem(itemName);
    if (!itemDef)
        return { success: false, error: `Unknown item: ${itemName}`, robot };
    const stats = computeEffectiveStats(robot);
    // Check money
...
export function sellItem(robot, instanceId) {
    const invItem = robot.inventory.find((i) => i.instanceId === instanceId);
    if (!invItem)
        return { success: false, error: "Item not found", robot, refund: 0 };
    const itemDef = lookupItem(invItem.itemName);
    if (!itemDef)
        return { success: false, error: "Unknown item", robot, refund: 0 };
