...
export function purple(s) { return `${c.purple}${s}${c.reset}`; }
...
export function turnHeader(turn) {
    return yellow(`════════════ TURN ${turn} ════════════`);
}
export function sectionHeader(title) {
    return yellow(`═══ ${title} ═══`);
}
export function formatBattleStatus(player, enemy) {
    const lines = [
...
export function formatCombatLog(log) {
    const lines = [];
...
export function formatVictorySummary(turnHistory, playerName, enemyName, rewards) {
    const lines = [
        "",
        green(bold("*** VICTORY! ***")),
        "",
