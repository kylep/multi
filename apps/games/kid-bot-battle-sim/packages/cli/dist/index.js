...
import { startGame, formatRobotStats, enterShop, getShopCatalog, buyItem, sellItem, getAvailableEnemies, startBattle, resolveTurn, endBattle, suggestPlayerAction, validateWeaponSelection, lookupItem, } from "@kid-bot-battle-sim/game-core";
import { purple, red, green, yellow, cyan, bold, dim, sectionHeader, turnHeader, formatBattleStatus, formatCombatLog, formatVictorySummary, formatDefeat, clearScreen, } from "./display.js";
const TITLE = `
 _  ___     _   ____        _
| |/ (_) __| | | __ )  ___ | |_
| ' /| |/ _\` | |  _ \\ / _ \\| __|
...
// ─── Battle ────────────────────────────────────────
async function battleLoop(state) {
    clearScreen();
    ...
    if (state.battle) {
        if (state.battle.outcome === "victory" && state.battle.rewards) {
            console.log(formatVictorySummary(state.battle.turnHistory, state.playerName, state.battle.enemyName, state.battle.rewards));
            console.log("\n" + formatRobotStats(state.robot, state.playerName));
        }
        else {
...
async function getPlayerAction(state) {
    const battle = state.battle;
...
async function getMainActionAfterConsumable(player, suggestion) {
    console.log("\n1. Attack");
    console.log("3. Rest");
...
async function getAttackAction(player, suggestion) {
